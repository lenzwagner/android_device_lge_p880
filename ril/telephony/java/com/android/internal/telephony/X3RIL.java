...
public class X3RIL extends RIL implements CommandsInterface {
    ...
    public void
    lgeSendCommand(int cmd) {
        //RIL_REQUEST_LGE_SEND_COMMAND
        RILRequest rrLSC = RILRequest.obtain(
                0x113, null);
        rrLSC.mParcel.writeInt(1);
        rrLSC.mParcel.writeInt(cmd);
        send(rrLSC);
        riljLog("X3RIL: LGE_SEND_COMAMND("+cmd+")");
    }

    public void
    lgeGoFastDormancy(int mode, int fdDelay, int scri) {
        //RIL_REQUEST_LGE_GO_FAST_DORMANCY
        RILRequest rrLSC = RILRequest.obtain(
                0xec, null);
        rrLSC.mParcel.writeInt(1);
        rrLSC.mParcel.writeInt(mode);
        rrLSC.mParcel.writeInt(fdDelay);
        rrLSC.mParcel.writeInt(scri);
        send(rrLSC);
        riljLog("X3RIL: LGE_GO_FAST_DORMANCY("+mode+","+fdDelay+","+scri+")");
